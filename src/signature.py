...
def create_300k_signature(ts: int, method: Method, path: str, api_secret: str, post_data: Optional[Dict] = None) -> str:
    ...
    signature = hmac.new(api_secret.encode(), f"{ts}{method.upper()}{path}{encode_body}".encode(), hashlib.sha256).hexdigest()

    return signature

def create_300k_header(method: Method, path: str, api_key: str, api_secret: str, post_data: Optional[Dict] = None, ts: Optional[int] = None):
